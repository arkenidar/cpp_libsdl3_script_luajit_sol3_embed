...
public:
    ...
    void setupLuaBindings() {
        ...
        lua["loadFont"] = [this](const std::string& path, float size) -> sol::object {
            int fontId = fontManager.loadFont(path, size);
            if (fontId < 0) {
                return sol::nil;
            }
        ...
        lua["setFont"] = [this](int fontId) -> bool {
            ...
            return fontManager.getCurrentFontId() == fontId;
        };

        lua["setFontSize"] = [this](float size) -> bool {
            if (fontManager.getCurrentFontId() == 0) return false;
            fontManager.setCurrentFontSize(size);
            return fontManager.getCurrentFont(size) != nullptr;
        };
        ...
        lua["measureText"] = [this](const std::string& text) -> sol::table {
            ...
            TTF_Font* font = fontManager.getCurrentFont(fontManager.getCurrentFontSize());
        ...
        lua["drawText"] = sol::overload(
            ...
            [this](const std::string& text, float x, float y, float r, float g, float b, float a) {
                TTF_Font* font = fontManager.getCurrentFont(fontManager.getCurrentFontSize());
                if (!font || !textEngine || !renderer) return;
            ...
            [this](const std::string& text, float x, float y, float size, float r, float g, float b, float a) {
                if (fontManager.getCurrentFontId() == 0 || !textEngine || !renderer) return;

                TTF_Font* font = fontManager.getFont(fontManager.getCurrentFontId(), size);
                if (!font) return;
        ...
        lua["createTextWidget"] = [this](sol::table config) -> sol::object {
            ...
            TTF_Font* font = fontManager.getCurrentFont(fontManager.getCurrentFontSize());
            widget->init(renderer, textEngine, font, window);
